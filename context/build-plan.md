...
### 10 JSearch Job Discovery (Pivot from Adzuna)

Agent calls JSearch API (via RapidAPI) to find jobs matching user's search criteria, scores them against user profile using Gemini, saves to DB.
...
- Call JSearch API:
  ...
  - params: `query={jobTitle} in {location}`, `page=1`, `num_pages=1`
- For each job returned (`data` array):
  - Extract title (`job_title`), company (`employer_name`), location (`job_city`, `job_country`), salary (format `job_min_salary` - `job_max_salary` if available), description snippet (`job_description`), redirect_url (`job_apply_link`)
  - Gemini reads the job description and scores it against user profile:
