...
**Check first:** Check AGENTS.md for an installed JSearch skill. If none exists — use this file and the RapidAPI docs.
...
Each JSearch job result contains:
...
type JSearchJob = {
  ...
  job_city: string;
  job_country: string;
  job_description: string; // usually full description
  job_apply_link: string;
  job_min_salary: number | null;
  job_max_salary: number | null;
  job_employment_type: string;
  job_posted_at_datetime_utc: string; // ISO date string
};
```
...
```typescript
// Map JSearch result to jobs table
const jobRecord = {
  user_id: userId,
  run_id: runId,
  source: "search", // always 'search' for discovered jobs
  ...
  about_role: job.job_description.substring(0, 300) + "...", // truncate for UI preview
...
- Always use the `x-rapidapi-key` header with `process.env.RAPIDAPI_KEY`.
- `source` is always `'search'` for JSearch jobs — never any other value.
- Gemini scores from `job.job_description` (which is often the full text).
